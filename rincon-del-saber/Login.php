...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($email === '' || $matricula === '') {
        ...
        mysqli_stmt_bind_param($consulta, 'ss', $email, $matricula);
        mysqli_stmt_execute($consulta);
        $resultado = mysqli_stmt_get_result($consulta);
        $estudiante = mysqli_fetch_assoc($resultado);

        if (!$estudiante) {
            $error = 'Email o matrícula incorrectos.';
        } else {
            ...
            $_SESSION['estudiante_matricula'] = $estudiante['matricula'];
            $_SESSION['estudiante_curso'] = $estudiante['curso'];
            $_SESSION['estudiante_email'] = $estudiante['email'];
...
<head>
    ...
    <title>Iniciar sesión · Rincón del Saber</title>
...
<body>
    <main class="aplicacion aplicacion--auth">
        ...
        <div class="auth-logo">
            ...
            <p>Ingresá con tu email y tu matrícula</p>
        </div>
        ...
        <form class="auth-formulario" action="Login.php" method="post" novalidate>
            ...
            <label class="modal__campo">
                Matrícula
                <input type="text" name="matricula" placeholder="104829" inputmode="numeric" required
        ...
        <p class="auth-alterno">
            ¿Sos profesor/a? <a href="LoginProfesor.php">Iniciá sesión acá</a>
        </p>
