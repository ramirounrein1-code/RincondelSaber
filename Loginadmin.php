...
/**
 * Loginadmin.php
 * Login exclusivo para el administrador del sistema. Usa su propia
 * clave de sesión (administrador_id) para no mezclarse con
 * estudiante_id / profesor_id, igual que Loginprofesor.php hace con
 * profesor_id.
 *
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($email === '' || $contrasena === '') {
        ...
        if (!$administrador) {
            ...
            $_SESSION['administrador_apellido'] = $administrador['Apellido'];
            $_SESSION['administrador_email'] = $administrador['email'];
...
<body>
    <main class="aplicacion aplicacion--auth" data-shell-fijo>
        ...
        <?php
        $tituloEncabezado = 'Panel de administración';
        ...
        <form class="auth-formulario" action="Loginadmin.php" method="post" novalidate>
            <label class="modal__campo">
                Email
                <input type="email" name="email" placeholder="admin@rincondelsaber.edu.ar" required autofocus
